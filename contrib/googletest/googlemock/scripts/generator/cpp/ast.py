...
class Node(object):
  ...
  def _StringHelper(self, name, suffix):
    return ('%s(%d, %d, %s)' % (name, self.start, self.end, suffix)
            if utils.DEBUG else f'{name}({suffix})')
...
class AstBuilder(object):
  ...
  def handle_return(self):
    tokens = self._GetTokensUpTo(tokenize.SYNTAX, ';')
    return (Return(tokens[0].start, tokens[0].end, tokens) if tokens else Return(
        self.current_token.start, self.current_token.end, None))
